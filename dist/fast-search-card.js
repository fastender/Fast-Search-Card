...
class FastSearchCard extends HTMLElement {
    ...
    async scanCustomFolders() {
        const basePath = '/config/www/fast-search-custom-folder/';
        
        try {
            // Versuche Metadata zu laden
            const metadata = await this.loadFolderMetadata();
            
            // Scan für tatsächlich existierende Ordner (später via API)
            // Für jetzt: Hardcoded Test-Ordner
            const existingFolders = ['rezepte', 'projekte', 'notizen'];
            
            return existingFolders.map(folder => ({
                type: 'file_collection',
                path: folder + '/',
                display_name: metadata?.folders?.[folder]?.display_name || this.formatFolderName(folder),
                icon: metadata?.folders?.[folder]?.icon || this.getDefaultFolderIcon(folder),
                folder_name: folder
            }));
            
        } catch (error) {
            console.warn('Could not scan custom folders:', error);
            return [];
        }
    }
    ...
    getDefaultFolderIcon(folder) {
        ...
        return iconMap[folder.toLowerCase()] || '📁';
    }
    
    async loadFolderMetadata() {
        try {
            // Später: echte File-API
            // Für jetzt: localStorage fallback
            const stored = localStorage.getItem('fast-search-folder-metadata');
    ...
    saveFolderMetadata(metadata) {
        try {
            // Später: echte File-API  
            // Für jetzt: localStorage
            localStorage.setItem('fast-search-folder-metadata', JSON.stringify(metadata));
        } catch (error) {
    ...
    async parseDynamicFolders(dataSource) {
        ...
        // Scan alle verfügbaren Ordner
        const folders = await this.scanCustomFolders();
        
        // Erstelle "Folder Manager" Item
        const folderManagerItem = {
            id: 'folder_manager',
            name: 'Ordner verwalten',
            domain: 'custom',
            category: 'custom',
            area: 'System',
            state: 'ready',
            attributes: {
                friendly_name: 'Ordner verwalten',
                custom_type: 'folder_manager'
            },
            icon: '🗂️',
            isActive: false,
            custom_data: {
        ...
        const folderItems = folders.map((folder, index) => ({
            ...
            state: 'available',
            attributes: {
                friendly_name: folder.display_name,
                custom_type: 'folder',
    ...
    async parseFileCollection(dataSource) {
        ...
        // Für jetzt: Mock-Files
        const mockFiles = [
            { name: 'pizza_margherita.md', content: '# 🍕 Pizza Margherita\n\n## Zutaten\n- Tomaten\n- Käse' },
            { name: 'pasta_carbonara.md', content: '# 🍝 Pasta Carbonara\n\n## Zutaten\n- Pasta\n- Eier' }
        ];
        
        return mockFiles.map((file, index) => ({
            id: `file_${dataSource.path}_${index}`,
            name: file.name.replace('.md', '').replace(/_/g, ' '),
            domain: 'custom',
            category: 'custom', 
            area: dataSource.display_name || 'Files',
            state: 'available',
            attributes: {
                friendly_name: file.name.replace('.md', ''),
                custom_type: 'file',
                file_path: `${dataSource.path}${file.name}`
            },
            icon: dataSource.icon || '📄',
            isActive: false,
            custom_data: {
                type: 'file',
                filename: file.name,
                filepath: `${dataSource.path}${file.name}`,
    ...
    scanFolderFiles(folderName) { // ← OHNE async!
        ...
        const mockFiles = {
            ...
            'projekte': ['kueche_renovierung.md', 'garten_umgestaltung.md'],
            'notizen': ['einkaufsliste.md', 'urlaubsplanung.md']
        };
        
        return mockFiles[folderName] || [];
    }
    ...
    parseCustomDataSource() {
        ...
        switch (dataSource.type) {
            ...
            case 'file_collection':           // ← NEU!
                return this.parseFileCollection(dataSource);
            case 'dynamic_folders':           // ← NEU!
                return this.parseDynamicFolders(dataSource);
            case 'calendar':
    ...
    getCustomDetailRightPaneHTML(item) {
        // NEU: Folder Manager
        if (item.custom_data?.type === 'folder_manager') {
            return this.getFolderManagerDetailHTML(item);
        }
        
        // NEU: Folder View  
        if (item.custom_data?.type === 'folder') {
            return this.getFolderDetailHTML(item);
        }
        
        // NEU: File View
        if (item.custom_data?.type === 'file') {
            return this.getFileDetailHTML(item);
        }
        
        // Check für Markdown Content (bestehend)
        if (item.custom_data?.type === 'input_select' && item.custom_data?.markdown_content) {
            return this.getMarkdownEditorDetailHTML(item);
        }
        
        // Fallback für andere Custom Types (bestehend)
        const tabsHTML = `
            <div class="detail-tabs-container desktop-tabs">
                <div class="detail-tabs">
                    <span class="tab-slider"></span>
    ...
    getFolderManagerDetailHTML(item) {
        const folders = item.custom_data.folders || [];
        
        return `
            <div class="folder-manager-container">
                <div class="folder-manager-header">
                    <h3>🗂️ Ordner verwalten</h3>
                    ...
                    ${folders.map(folder => `
                        <div class="folder-item" data-folder="${folder.folder_name}">
                            ...
                            <div class="folder-info">
                                <div class="folder-name">${folder.display_name}</div>
                                <div class="folder-path">${folder.path}</div>
                            </div>
                            <div class="folder-actions">
                                <button class="folder-action-btn" data-action="edit-folder" title="Bearbeiten">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                                    </svg>
                                </button>
                                <button class="folder-action-btn delete" data-action="delete-folder" title="Löschen">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="add-folder-form hidden">
                    <div class="form-group">
                        <label>Ordner Name:</label>
                        <input type="text" class="folder-name-input" placeholder="z.B. meine_rezepte">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-actions">
                        <button class="btn-secondary" data-action="cancel-add">Abbrechen</button>
                        <button class="btn-primary" data-action="confirm-add">Erstellen</button>
                    </div>
                </div>
            </div>
    ...
    setupCustomDetailTabs(item) {
        ...
        // NEU: Folder Manager Event Listeners
        if (item.custom_data?.type === 'folder_manager') {
            this.setupFolderManagerEvents(item);
        }
    ...
    setupFolderManagerEvents(item) {
        const addFolderBtn = this.shadowRoot.querySelector('.add-folder-btn');
        const addFolderForm = this.shadowRoot.querySelector('.add-folder-form');
        const folderNameInput = this.shadowRoot.querySelector('.folder-name-input');
        const iconOptions = this.shadowRoot.querySelectorAll('.icon-option');
        const confirmBtn = this.shadowRoot.querySelector('[data-action="confirm-add"]');
        const cancelBtn = this.shadowRoot.querySelector('[data-action="cancel-add"]');
        ...
        // Icon Selection
        iconOptions.forEach(iconBtn => {
            iconBtn.addEventListener('click', () => {
                // Remove previous selection
                iconOptions.forEach(btn => btn.classList.remove('selected'));
        ...
        if (confirmBtn) {
            confirmBtn.addEventListener('click', () => {
                ...
                if (!/^[a-zA-Z0-9_-]+$/.test(folderName)) {
                    alert('Ordnername darf nur Buchstaben, Zahlen, _ und - enthalten!');
                    return;
                }
                
                this.createNewFolder(folderName, selectedIcon);
            });
        }
        ...
        const folderActionBtns = this.shadowRoot.querySelectorAll('.folder-action-btn');
        folderActionBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.handleFolderAction(action, folderName);
            });
        });
    }
    
    createNewFolder(folderName, icon) {
        console.log(`🗂️ Creating new folder: ${folderName} with icon: ${icon}`);
        
        // Update metadata
        const metadata = this.loadFolderMetadataSync() || { folders: {} };
        metadata.folders[folderName] = {
            display_name: this.formatFolderName(folderName),
            icon: icon
        };
        
        this.saveFolderMetadata(metadata);
        
        // Show success message
        this.showFolderMessage('✅ Ordner erstellt!', 'success');
    ...
    editFolder(folderName) {
        const newName = prompt(`Neuer Name für "${folderName}":`, folderName);
        if (newName && newName !== folderName) {
            console.log(`✏️ Renaming folder: ${folderName} → ${newName}`);
            
            const metadata = this.loadFolderMetadataSync() || { folders: {} };
            if (metadata.folders[folderName]) {
                metadata.folders[newName] = metadata.folders[folderName];
                metadata.folders[newName].display_name = this.formatFolderName(newName);
                delete metadata.folders[folderName];
                
                this.saveFolderMetadata(metadata);
                this.showFolderMessage('✅ Ordner umbenannt!', 'success');
                
                setTimeout(() => this.refreshCustomItems(), 1000);
            }
        }
    }
    
    deleteFolder(folderName) {
        if (confirm(`Ordner "${folderName}" wirklich löschen?\n\n⚠️ Alle Dateien gehen verloren!`)) {
            ...
            this.saveFolderMetadata(metadata);
            this.showFolderMessage('🗑️ Ordner gelöscht!', 'warning');
            
            setTimeout(() => this.refreshCustomItems(), 1000);
        }
    }
    
    loadFolderMetadataSync() {
        try {
            const stored = localStorage.getItem('fast-search-folder-metadata');
            return stored ? JSON.parse(stored) : null;
        } catch (error) {
    ...
    showFolderMessage(message, type) {
        // Simple message display
        const existingMsg = this.shadowRoot.querySelector('.folder-message');
        if (existingMsg) existingMsg.remove();
        
        const messageDiv = document.createElement('div');
        messageDiv.className = 'folder-message';
    ...
    refreshCustomItems() {
        // Trigger updateItems to reload everything
        this.updateItems();
